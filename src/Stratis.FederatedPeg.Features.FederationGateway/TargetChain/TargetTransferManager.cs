...
{
    ///<inheritdoc/>
    internal class TargetTransferManager : ITargetTransferManager
    {
        ...
        private void RegisterSession(int blockHeight, List<TargetTransferRequest> counterChainTransactionInfos)
        {
            ...
            counterChainSession.Transfers = counterChainTransactionInfos.Select(c => new Transfer
            {
                SourceBlockNumber = blockHeight,
                SourceTransactionId = c.TransactionId,
                TargetAddress = c.TargetAddress,
                SourceDepositAmount = c.Amount
            }).ToList();
        ...
        public void ReceivePartial(int blockHeight, Transaction partialTransaction, uint256 bossCard)
        {
            ...
            var counterChainSession = this.sessions[blockHeight];
        ...
        public void CreateTransferOnTargetChain(int blockHeight, List<TargetTransferRequest> counterChainTransactionInfos)
        {
        ...
        ///<inheritdoc/>
        public async Task<uint256> ProcessTransfers(int blockHeight)
        {
            ...
            if (this.sessions.TryGetValue(blockHeight, out var counterchainSession))
            {
                ...
                if (counterchainSession.TargetTransactionId != uint256.Zero)
                {
                    // If we get here:
                    ...
                    return counterchainSession.TargetTransactionId;
                }
            }
            else
            {
            ...
            var recipients = counterchainSession.Transfers.Select(s =>
                new Recipient.Recipient
                {
                    Amount = s.SourceDepositAmount,
                    ScriptPubKey = BitcoinAddress.Create(s.TargetAddress, this.network).ScriptPubKey
                }).ToList();
            ...
            if (counterChainSession == null)
            {
                ...
                if (exists) return counterChainSession.TargetTransactionId;
                throw new InvalidOperationException($"No CounterChainSession found in the counter chain for block height {blockHeight}.");
            }
            ...
            var federationNetworkPeers =
                ...
                .Where(p => !p.Inbound && this.federationGatewaySettings.FederationNodeIpEndPoints.Any(e => this.ipAddressComparer.Equals(e.Address, p.PeerEndPoint.Address)));
        ...
        public CounterChainSession VerifySession(int blockHeight, Transaction partialTransactionTemplate)
        {
            ...
            var allAddressesInSession = counterChainSession.Transfers.Select(
                trxInfo => new
                {
                    Address = BitcoinAddress.Create(trxInfo.TargetAddress, this.network).ScriptPubKey,
                    Amount = trxInfo.SourceDepositAmount
                }).ToList();
            ...
            foreach (var amount in amountByAddress)
            {
                ...
                if (!match)
                {
                    this.logger.LogInformation("Session for block {0} found mismatch in amounts.", blockHeight);
                    return null;
                }
            }
        ...
        public void AddCounterChainTransactionId(int blockHeight, uint256 transactionId)
        {
            ...
            if (counterChainSession.TargetTransactionId != uint256.Zero)
            {
                this.logger.LogInformation($"Session::AddCounterChainTransactionId: Attempting to write transactionId {transactionId} while session already has transactionId {counterChainSession.TargetTransactionId}. Fatal!");
                return;
            }

            counterChainSession.TargetTransactionId = transactionId;
